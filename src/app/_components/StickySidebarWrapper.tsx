...
export function StickySidebarWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <aside
      className={`hidden lg:flex w-80 flex-col p-10 gap-10 bg-white/60 backdrop-blur-xl border border-primary/5 rounded-[2.5rem] h-fit overflow-y-auto no-scrollbar sticky shadow-2xl shadow-primary/5 transition-all duration-700 ease-in-out z-30 transform-gpu will-change-transform touch-manipulation ${
        headerVisible
