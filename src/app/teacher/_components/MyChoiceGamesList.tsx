...
export function MyChoiceGamesList() {
  ...
  const [games, setGames] = useState<ChoiceShooterGame[]>([]);
  const [shareGame, setShareGame] = useState<ChoiceShooterGame | null>(null);
  const [loading, setLoading] = useState(true);

  const loadGames = () => {
    const list = getChoiceShooterGames();
    setGames(list);
    setLoading(false);
  };
  ...
  const handleDelete = (gameId: string, gameTitle: string) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa bài tập "${gameTitle}" không?`)) return;

    const ok = deleteChoiceShooterGame(gameId);
    if (ok) {
      toast.success(`Đã xóa bài tập "${gameTitle}"!`);
      loadGames();
    } else {
        ...
        <div className="w-full py-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-emerald-500/10 p-8 flex flex-col items-center justify-center text-center space-y-4 shadow-sm">
          ...
          <div className="space-y-1 max-w-md">
            <h3 className="font-headline font-black text-xl text-slate-800 dark:text-white">
              Bạn chưa tạo bài tập Bắn súng Chọn đáp án nào
            </h3>
            <p className="text-xs text-slate-400 font-medium">
              Hãy tạo các bài tập Bắn súng Toán học tự động để học sinh rèn luyện tính nhẩm tốc độ!
            </p>
          </div>
          <button
            type="button"
          ...
          {games.map((game) => (
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                ...
                <Link
                  href={`/teacher/games/choice-shooter/create?id=${game.id}`}
