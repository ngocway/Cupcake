...
function FlipGameCardItem({ game }: { game: GameCard }) {
        ...
        {isPlaying ? (
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-14 h-14 rounded-full bg-white/90 dark:bg-slate-900/90 text-rose-500 flex items-center justify-center shadow-2xl border-2 border-white/80 group-hover/thumb:scale-115 group-hover/thumb:bg-rose-500 group-hover/thumb:text-white transition-all duration-300">
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        {FLIP_GAMES.map((game) => (
          <FlipGameCardItem key={game.id} game={game} />
        ))}
      </div>
    </div>
  );
