            ...
            >
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                <a
                  ...
                  title="Chơi thử game Nối Cặp Flashcard Match trong tab mới"
                >
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all shrink-0 border border-slate-200/60 dark:border-slate-700 ${
                    deletingId === topic.id
                      ? "bg-rose-50 dark:bg-rose-950/50 border-rose-200 text-rose-500 cursor-not-allowed opacity-90"
                      : "bg-slate-100 hover:bg-rose-50 hover:text-rose-600 dark:bg-slate-800 dark:hover:bg-rose-950/40 text-slate-400 cursor-pointer"
                  }`}
              ...
              <div className="pt-2.5 flex items-center gap-2 border-t border-slate-100 dark:border-slate-800">
                <div className="flex-1 min-w-0 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800/60 px-3 py-1.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60">
                  <Link2 className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                  <span className="text-[11px] font-mono text-slate-500 dark:text-slate-400 truncate">
                    {typeof window !== "undefined"
                      ? `${window.location.origin}/student/game/flashcard-match?topicId=${topic.id}`
                      : `/student/game/flashcard-match?topicId=${topic.id}`}
                  </span>
                </div>

                <button
                  onClick={() => setShareTopic(topic)}
                  className="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs rounded-xl shadow-md shadow-purple-600/20 transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer shrink-0"
