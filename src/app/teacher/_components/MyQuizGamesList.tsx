...
export function MyQuizGamesList({ initialTopics }: { initialTopics?: any[] }) {
  ...
  const handleCreateNew = () => {
    router.push("/teacher/games/candy-quiz/create");
  };
  ...
  const gamePath = "/student/game/candy-quiz";
    ...
    <div className="w-full space-y-6 animate-in fade-in duration-300">
      {topics.length === 0 ? (
        ...
        <div className="w-full py-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl border border-primary/10 p-8 flex flex-col items-center justify-center text-center space-y-4 shadow-sm">
          ...
          <div className="space-y-1 max-w-md">
            ...
            <p className="text-xs text-slate-400 font-medium">
              Hãy tạo bài tập Trắc nghiệm Kẹo Ngọt đầu tiên để học sinh ôn luyện câu hỏi và đáp án dài với giao diện hấp dẫn!
            </p>
          </div>
          <button
            type="button"
            onClick={handleCreateNew}
            className="px-6 py-3 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-lg shadow-pink-500/20 active:scale-95 transition-all flex items-center gap-2 cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {topics.map((topic) => (
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <h3 className="font-headline font-black text-lg line-clamp-2 leading-snug text-pink-700 dark:text-pink-300">
                  {topic.name}
                </h3>
              </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
                {topic.items?.slice(0, 4).map((item: any) => (
                  <div
                    key={item.id}
                    className="h-12 rounded-xl border shrink-0 p-1 flex items-center justify-center overflow-hidden gap-1 bg-pink-50/80 dark:bg-pink-950/30 border-pink-200/80 dark:border-pink-800/60"
                    title={item.word}
                  >
                    {item.imageUrl ? (
                      <img
                      ...
                      <div className="flex flex-col items-center justify-center px-2 max-w-[110px]">
                        <span className="text-[9px] font-black text-pink-900 dark:text-pink-200 truncate w-full text-center">
                          {item.word}
                        </span>
                      </div>
                    )}
                ...
                {(topic.totalItems || topic.items?.length || 0) > 4 && (
                  <div className="w-8 h-12 rounded-xl font-bold text-xs flex items-center justify-center shrink-0 bg-pink-500/20 text-pink-700 dark:text-pink-300">
                    +{(topic.totalItems || topic.items?.length || 0) - 4}
                  </div>
                )}
              </div>
              ...
              {/* Card Footer Actions */}
              <div className="pt-3 border-t flex items-center justify-between gap-2 border-pink-200/50 dark:border-pink-800/50">
                <a
                  href={`${gamePath}?topicId=${topic.id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 py-2 px-3 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white font-bold text-xs rounded-xl shadow-md shadow-pink-500/20 transition-all flex items-center justify-center gap-1.5 active:scale-95 cursor-pointer"
                ...
                <Link
                  href={`/teacher/games/candy-quiz/create?topicId=${topic.id}`}
                  className="py-2 px-3 font-bold text-xs rounded-xl transition-all flex items-center gap-1 active:scale-95 cursor-pointer border bg-slate-50 dark:bg-slate-800 hover:bg-pink-50 text-pink-800 dark:text-pink-200 border-pink-200/60 dark:border-pink-700"
                  title="Chỉnh sửa bài tập"
                >
                  <Edit3 className="w-3.5 h-3.5 shrink-0 text-pink-600" />
                  <span>Sửa</span>
                </Link>
                ...
                <button
                  ...
                  className="w-9 h-9 rounded-xl flex items-center justify-center transition-all shrink-0 border bg-slate-50 hover:bg-rose-50 hover:text-rose-600 dark:bg-slate-800 dark:hover:bg-rose-950/40 text-slate-400 border-pink-200/60 dark:border-pink-700 cursor-pointer disabled:opacity-50"
              ...
              <div className="pt-2.5 flex items-center gap-2 border-t border-pink-200/50 dark:border-pink-800/50">
                <div className="flex-1 min-w-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl border bg-pink-50/60 dark:bg-slate-800/60 border-pink-200/60 dark:border-pink-700/60">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-white font-bold text-xs rounded-xl shadow-md shadow-pink-500/20 transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer shrink-0"
          ...
          <div
            onClick={handleCreateNew}
            className="min-h-[220px] rounded-3xl border-2 border-dashed border-pink-300 hover:border-pink-500 bg-pink-50/40 hover:bg-pink-50 dark:bg-slate-900/40 dark:hover:bg-slate-900 transition-all duration-300 flex flex-col items-center justify-center text-center p-6 cursor-pointer group hover:-translate-y-1 shadow-sm hover:shadow-md"
          >
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-r from-pink-500 to-rose-600 text-white flex items-center justify-center mb-3 group-hover:scale-110 transition-transform shadow-md shadow-pink-500/30">
            ...
            <span className="text-xs font-semibold text-slate-400 mt-1">
              Thêm game Trắc nghiệm Kẹo Ngọt
            </span>
          </div>
        </div>
      )}
      ...
      {shareTopic && (
        <MatchGameShareModal
          ...
          gamePath={shareTopic.customGamePath || gamePath}
        />
      )}
    </div>
  );
