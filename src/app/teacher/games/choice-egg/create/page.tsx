...
export default function CreateChoiceEggPage() {
  ...
  // Form States (Step 1)
  const [gameTitle, setGameTitle] = useState("Bài tập Đập Trứng Toán học");
  ...
  const [createdCode, setCreatedCode] = useState(() => `EGG-${Math.floor(1000 + Math.random() * 9000)}`);

  // Load existing game if editing
  useEffect(() => {
    if (editId) {
      const existing = getChoiceEggGameById(editId);
      if (existing) {
  ...
  const handleCopyLink = () => {
    const link = `${window.location.origin}/game/egg-smash/${createdCode}`;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-headline font-black text-2xl md:text-3xl text-slate-800 dark:text-white tracking-tight">
                Tạo Game Đập Trứng Toán Học
              </h1>
              <p className="text-xs font-semibold text-slate-400 mt-0.5">
                Hệ thống tự động sinh câu hỏi đập trứng theo các dạng toán được chọn
              </p>
            </div>
          </div>
              ...
              <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-slate-200/80 dark:border-slate-800 shadow-xl space-y-6">
                <div className="w-full space-y-2">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-2xl font-bold text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all text-sm"
                  />
                </div>

                {/* Math Types Selection Grid */}
                <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 px-3 py-1 rounded-full border border-amber-200/60 dark:border-amber-800">
                      ...
                      return (
                        <div
                          ...
                          className={`p-5 rounded-3xl border-2 transition-all cursor-pointer flex flex-col justify-between gap-4 relative overflow-hidden group ${
                            isSelected
                              ? "bg-gradient-to-br from-amber-50/90 to-orange-50/50 dark:from-amber-950/40 dark:to-orange-950/20 border-amber-500 shadow-xl shadow-amber-500/10 scale-[1.02]"
                        ...
                        >
                          ...
                          <div
                            className={`absolute -right-8 -top-8 w-24 h-24 rounded-full blur-2xl transition-opacity ${
                              isSelected ? "opacity-30 bg-amber-400" : "opacity-0"
                            }`}
                          />
                          ...
                          <div className="flex items-center justify-between relative z-10">
                            ...
                            <div
                              className={`w-7 h-7 rounded-2xl border-2 flex items-center justify-center transition-all ${
                                isSelected
                                  ? "bg-amber-500 border-amber-500 text-white shadow-md shadow-amber-500/30 scale-110"
                          ...
                          <div className="relative z-10 space-y-2">
                            <h4 className="font-headline font-black text-base text-slate-800 dark:text-slate-100 group-hover:text-amber-600 transition-colors">
                              {opt.title}
                            </h4>
                ...
                {/* Toolbox Card: Question Count & End Mode */}
                  ...
                  <div className="bg-slate-50/90 dark:bg-slate-800/60 rounded-3xl p-5 border border-slate-200/80 dark:border-slate-700/80 space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <span className="w-7 h-7 rounded-xl bg-amber-100 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 flex items-center justify-center font-bold text-xs">
                          🥚
                        </span>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="relative w-16">
                          <input
                            ...
                            className="w-full px-2 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-mono font-bold text-slate-800 dark:text-white text-xs text-center focus:outline-none focus:ring-2 focus:ring-amber-500"
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div className="lg:col-span-4">
              <div className="sticky top-6 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-2xl space-y-6">
                <div className="flex items-center gap-3 pb-4 border-b border-slate-100 dark:border-slate-800">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-amber-500 to-orange-400 text-white flex items-center justify-center shadow-lg shadow-amber-500/30 text-xl font-bold">
                    🥚
                  </div>
                  <div>
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="w-full py-4 px-6 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 active:scale-95 disabled:opacity-50 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-amber-500/25 transition-all flex items-center justify-center gap-2 group cursor-pointer"
                  >
          ...
          <div className="space-y-6 animate-in fade-in duration-300">
            <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                <h2 className="font-headline font-black text-xl text-slate-800 dark:text-white flex items-center gap-2">
                  ...
                  <span className="text-xs bg-amber-100 text-amber-700 font-bold px-2.5 py-0.5 rounded-full">
                    Đã sinh tự động
                  </span>
                </h2>
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                <button
                  ...
                  className="flex-1 md:flex-initial py-3.5 px-6 bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 active:scale-95 text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-amber-500/25 transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
              {questions.map((item, idx) => (
                <div
                  key={item.id}
                  className="bg-white/95 dark:bg-slate-900/90 backdrop-blur-md rounded-3xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-md flex flex-col justify-between gap-4 group hover:border-amber-500/40 transition-all relative overflow-hidden"
                >
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 md:p-8 max-w-md w-full border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6 text-center animate-in zoom-in-95 duration-200 relative">
            ...
            <div className="w-16 h-16 rounded-3xl bg-gradient-to-tr from-amber-500 to-orange-400 text-white flex items-center justify-center mx-auto shadow-lg shadow-amber-500/30 text-3xl font-bold">
              🥚
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs font-semibold text-slate-400 px-2">
                Bài tập "{gameTitle}" đã sẵn sàng cho học sinh tham gia làm bài
              </p>
            </div>

            <div className="bg-amber-50/60 dark:bg-amber-950/30 rounded-2xl p-3.5 border border-amber-200/60 dark:border-amber-800/60 space-y-0.5">
            ...
            <div className="space-y-2 text-left">
              ...
              <div className="flex gap-2">
                <input
                  type="text"
                  readOnly
                  value={`${typeof window !== "undefined" ? window.location.origin : ""}/game/egg-smash/${createdCode}`}
                ...
                <button
                  ...
                  className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 active:scale-95 text-white font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 shrink-0 shadow-md shadow-amber-500/20 cursor-pointer whitespace-nowrap"
                >
            ...
            <div className="pt-2 flex flex-col gap-2">
              <Link
                href={`/game/egg-smash/${createdCode}`}
                target="_blank"
                className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md shadow-amber-500/20 text-center"
              >
                Chơi thử ngay ➔
              </Link>
              <button
                type="button"
                onClick={() => router.push("/teacher?tab=choice")}
                className="w-full py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-xs uppercase tracking-wider rounded-xl transition-all cursor-pointer"
              >
