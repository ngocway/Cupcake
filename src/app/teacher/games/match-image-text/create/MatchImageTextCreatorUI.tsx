...
export function MatchImageTextCreatorUI({ gameType }: { gameType: string }) {
  ...
  const handleApplyAutoGenerated = ({
    ...
    // 1. Tự động điền tiêu đề bài tập nếu đang trống
    if (!title.trim() && suggestedTitle) {
      setTitle(suggestedTitle);
      setTitleError(false);
    }

    // 2. Thêm nối tiếp vào danh sách thẻ hiện có:
    // Kiểm tra xem vòng 1 có phải chỉ có duy nhất 1 thẻ rỗng ban đầu không
    const isSingleEmptyInitial =
      rounds.length === 1 &&
      rounds[0].pairs.length === 1 &&
      !rounds[0].pairs[0].word.trim() &&
      !rounds[0].pairs[0].imageUrl;
      ...
      const updatedPairs = await Promise.all(
        allPairs.map(async (pair) => {
          const [finalImageUrl, finalAudioUrl] = await Promise.all([
            ...
            // Task B: Process Audio File (Custom File) or Synthesize Edge TTS Child Voice
            (async () => {
              if (pair.audioFile) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-100 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-3.5 py-2 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold text-xs rounded-2xl shadow-md shadow-purple-500/25 transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer shrink-0"
                >
