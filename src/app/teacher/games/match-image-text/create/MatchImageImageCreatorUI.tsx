...
export function MatchImageImageCreatorUI() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Failed to generate match image-image thumbnail:", thumbErr);
      }
  ...
  const handleApplyAutoGenerated = ({
    ...
    if (!generatedPairs || generatedPairs.length === 0) return;
    ...
    const isSingleEmptyInitial =
      rounds.length === 1 &&
      rounds[0].pairs.length === 1 &&
      !rounds[0].pairs[0].imageAUrl &&
      !rounds[0].pairs[0].imageBUrl;

    let updatedRounds: GameRound[] = isSingleEmptyInitial
      ? []
      : rounds.map(r => ({ ...r, pairs: [...r.pairs] }));
      ...
      if (spaceLeft > 0) {
        ...
        lastRound.pairs.push(
          ...toAdd.map((item, idx) => ({
            ...
            imageAUrl: item.imageAUrl || item.imageUrl,
            imageBUrl: item.imageBUrl || item.imageUrl,
          }))
        );
        remainingPairs = remainingPairs.slice(spaceLeft);
      }
    }
    ...
    while (remainingPairs.length > 0 && updatedRounds.length < MAX_ROUNDS) {
      ...
      const newPairs: ImageCardPair[] = chunk.map((item, idx) => ({
      ...
      updatedRounds.push({
        id: `round-ai-${roundIdx}-${Date.now()}`,
        title: `Vòng ${roundIdx + 1}`,
        pairs: newPairs,
      });
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-300 pb-16">
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 hover:from-purple-700 hover:to-indigo-800 text-white font-bold text-xs rounded-2xl shadow-md shadow-purple-500/25 transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer shrink-0"
              >
                <Sparkles className="w-4 h-4 stroke-[2] text-amber-300 animate-pulse" />
      ...
      {/* --- AUTO GENERATE TOPIC MODAL --- */}
      <AutoGenerateTopicModal
