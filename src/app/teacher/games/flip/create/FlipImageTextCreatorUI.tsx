...
export function FlipImageTextCreatorUI({ gameType }: { gameType: string }) {
  ...
  const handleAddPair = () => {
    if (pairs.length >= MAX_PAIRS_PER_ROUND) {
      toast.error(`Vòng ${activeRoundIndex + 1} đã đạt tối đa 8 cặp thẻ. Vui lòng bấm [+ Thêm Vòng Mới] để tạo vòng tiếp theo!`);
      return;
    }
  ...
  const handleSave = async () => {
    ...
    for (let rIdx = 0; rIdx < rounds.length; rIdx++) {
      ...
      if (r.pairs.length < 2) {
        ...
        setValidationModalMessage(`${r.title} phải có ít nhất 2 cặp thẻ để học sinh lật ghép. Vui lòng bấm [+ Thêm 1 cặp]!`);
        return;
      }
    }
    ...
    for (let rIdx = 0; rIdx < rounds.length; rIdx++) {
      ...
      for (let pIdx = 0; pIdx < r.pairs.length; pIdx++) {
        ...
        if (count > MAX_TEXT_WORDS) {
          setActiveRoundIndex(rIdx);
          setValidationModalMessage(`${r.title}, Cặp thẻ thứ #${pIdx + 1} ("${p.word.slice(0, 20)}...") đang có ${count} từ (vượt quá 5 từ cho game Lật Ảnh-Chữ). Vui lòng rút gọn!`);
          return;
        }
      }
    }

    // Verification 4: Check for missing audio files in CUSTOM_FILE mode
    ...
    // Verification 5: Check for duplicate words across all rounds
    ...
    try {
      // Parallel batch processing: process all pairs simultaneously
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300 border border-orange-200 dark:border-orange-800">
                Lật Ảnh-Chữ
              </span>
            ...
            <h1 className="font-headline font-black text-2xl md:text-3xl text-slate-800 dark:text-white mt-1">
              {topicId ? "Chỉnh sửa Game Lật Ảnh-Chữ" : "Tạo Mới Game Lật Ảnh-Chữ"}
            </h1>
          </div>
        </div>
      </div>
      ...
      {isLoadingTopic ? (
        <div className="w-full min-h-[420px] bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl border border-primary/10 p-12 flex flex-col items-center justify-center text-center shadow-sm space-y-4 animate-in fade-in duration-300">
          <div className="relative flex items-center justify-center">
            <div className="w-16 h-16 rounded-2xl bg-orange-100 dark:bg-orange-900/40 text-orange-600 flex items-center justify-center shadow-inner">
        ...
        <>
          ...
          <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-3xl border border-primary/10 p-6 md:p-8 shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Nhập tên bài tập (VD: Bài tập Lật Ảnh-Chữ vựng tiếng Anh)..."
          ...
          <div className="relative bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-3xl border border-primary/10 p-6 md:p-8 shadow-sm space-y-3.5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-100 dark:border-slate-800">
              <div>
                <h2 className="font-headline font-black text-lg text-slate-800 dark:text-white">
                  2. Danh sách các cặp thẻ Lật (Ảnh - Chữ)
                </h2>
              </div>
            ...
            <div className="flex items-center justify-between gap-3 pt-2">
              <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none max-w-full">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-300 hover:bg-orange-100 rounded-2xl text-xs font-bold border border-orange-200 dark:border-orange-800 transition-all flex items-center gap-1 shrink-0 cursor-pointer"
                >
            ...
            <div className="p-3.5 bg-orange-50/70 dark:bg-orange-950/30 rounded-2xl border border-orange-200/60 dark:border-orange-800/60 flex items-start gap-2.5 text-xs text-orange-900 dark:text-orange-200">
              <Info className="w-4 h-4 text-orange-500 shrink-0 mt-0.5" />
              <p className="leading-relaxed">
                <strong>Hướng dẫn phân bổ Vòng chơi:</strong> Mỗi Vòng chứa tối đa <strong>8 cặp thẻ</strong> để Học sinh lật ghép không bị rối mắt. Khi Học sinh hoàn thành Vòng 1, game sẽ tự động chuyển tiếp sang Vòng 2!
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-2">
              {pairs.map((pair, index) => (
                <div 
                  key={pair.id}
                  className="group relative bg-white dark:bg-slate-800/90 rounded-2xl border-2 border-slate-200/80 dark:border-slate-700 p-4 space-y-3 shadow-sm hover:border-orange-300 dark:hover:border-orange-600 hover:shadow-md transition-all duration-200 flex flex-col justify-between"
                >
                  {/* Pair Header & Delete */}
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700/60 pb-2">
                    <span className="w-6 h-6 rounded-full bg-orange-500 text-white font-black text-xs flex items-center justify-center shadow-sm">
                      {index + 1}
                    </span>
                    {pairs.length > 1 && (
                      <button
                  ...
                  <div 
                    ...
                    className={`relative aspect-square rounded-[4px] bg-slate-50 dark:bg-slate-900 border-2 border-dashed overflow-hidden flex items-center justify-center transition-all ${
                      dragActivePairId === pair.id 
                        ? "border-orange-500 bg-orange-50 dark:bg-orange-950/60 scale-[1.02]" 
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-xl text-xs font-black text-center focus:ring-2 outline-none transition-all ${
                        countWords(pair.word) > MAX_TEXT_WORDS
                          ...
                          : "bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 focus:ring-orange-500/20 focus:border-orange-500"
                      }`}
                    />
                    {countWords(pair.word) > MAX_TEXT_WORDS && (
                  ...
                  {audioMode === "AUTO_TTS" && (
                    <div className="pt-1 flex justify-center">
                      <button
                        ...
                        className="text-[11px] font-bold text-orange-600 dark:text-orange-400 hover:underline flex items-center gap-1 disabled:opacity-30 disabled:no-underline cursor-pointer"
                      >
                        <Volume2 className="w-3 h-3" />
              ...
              <div 
                ...
                className={`group border-2 border-dashed border-orange-300 dark:border-orange-800 hover:border-orange-500 bg-orange-50/40 dark:bg-orange-950/20 hover:bg-orange-50 dark:hover:bg-orange-950/40 rounded-2xl p-6 flex flex-col items-center justify-center text-center space-y-2 transition-all cursor-pointer min-h-[260px] ${
                  pairs.length >= MAX_PAIRS_PER_ROUND ? "opacity-50 pointer-events-none" : ""
                }`}
              >
                <div className="w-12 h-12 rounded-full bg-orange-500 text-white flex items-center justify-center shadow-lg shadow-orange-500/30 group-hover:scale-110 transition-transform">
                  <Plus className="w-6 h-6 stroke-[3]" />
                </div>
                <span className="font-headline font-black text-sm text-orange-900 dark:text-orange-200">Thêm 1 cặp</span>
                <span className="text-[11px] font-bold text-orange-600/70 dark:text-orange-400/70">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-slate-400">
                      Tối đa 70 từ. Hệ thống tự động chia thành các Vòng chơi (Mỗi vòng 8 từ).
                    </p>
                  </div>
                </div>
