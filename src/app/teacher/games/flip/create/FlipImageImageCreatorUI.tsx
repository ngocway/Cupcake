...
export function FlipImageImageCreatorUI() {
  ...
  const handleAddPair = () => {
    if (pairs.length >= MAX_PAIRS_PER_ROUND) {
      setValidationModalMessage(`Vòng ${activeRoundIndex + 1} đã đạt tối đa 8 cặp thẻ. Vui lòng bấm [+ Thêm Vòng Mới] để tạo vòng tiếp theo!`);
      return;
    }
  ...
  const handleMovePairToRound = (pairId: string, targetRoundIndex: number) => {
    ...
    if (targetRound.pairs.length >= MAX_PAIRS_PER_ROUND) {
      toast.error(`${targetRound.title} đã có 8 cặp thẻ (đã đầy)!`, { position: "top-center" });
      return;
    }
  ...
  const handleSingleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Optimistically set preview URL & loading state
  ...
  const handleBulkImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Fast async background uploads for each pair
  ...
  const handleDropImage = async (e: React.DragEvent, targetPairId: string, targetSide: "A" | "B") => {
    e.preventDefault();
    setDragActiveTarget(null);
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      setRounds(prev => prev.map((r, rIdx) => rIdx !== activeRoundIndex ? r : {
        ...r,
        pairs: r.pairs.map(p => p.id === targetPairId ? (
          targetSide === "A"
            ? { ...p, imageAUrl: tempUrl, imageAFile: file, isUploadingA: true }
            : { ...p, imageBUrl: tempUrl, imageBFile: file, isUploadingB: true }
        ) : p)
      ...
      try {
        ...
        toast.success(`Đã nạp & nén ảnh vế ${targetSide} thành công!`);
      } catch (err: any) {
        toast.error(`Nạp ảnh vế ${targetSide} thất bại: ${err.message}`);
        setRounds(prev => prev.map((r, rIdx) => rIdx !== activeRoundIndex ? r : {
          ...r,
          pairs: r.pairs.map(p => p.id === targetPairId ? (
            targetSide === "A" ? { ...p, isUploadingA: false } : { ...p, isUploadingB: false }
          ) : p)
        }));
      }
      setDragSource(null);
  ...
  const handleSave = async () => {
    ...
    for (let rIdx = 0; rIdx < rounds.length; rIdx++) {
      ...
      if (r.pairs.length < 2) {
        ...
        setValidationModalMessage(`${r.title} phải có ít nhất 2 cặp ảnh để học sinh lật ghép. Vui lòng bấm [+ Thêm 1 cặp]!`);
        return;
      }
    }
    ...
    // Verification 3: Check any card is currently uploading
    const isAnyUploading = rounds.some(r => r.pairs.some(p => p.isUploadingA || p.isUploadingB));
    if (isAnyUploading) {
      toast.info("Một số hình ảnh đang được nén và tải lên trong nền. Vui lòng đợi vài giây...");
    }

    setIsSaving(true);
    toast.loading("Đang lưu bài tập Lật Ảnh-Ảnh...", { id: "save-game-toast" });

    try {
      // Process remaining un-uploaded files if any exist
      ...
      try {
        ...
        console.warn("Failed to generate flip image-image thumbnail:", thumbErr);
      }
  ...
  if (isLoadingTopic) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center">
        ...
        <p className="text-sm font-semibold text-slate-500 animate-pulse">Đang tải dữ liệu bài tập Lật Ảnh-Ảnh...</p>
      </div>
    );
  }
    ...
    <div className="w-full space-y-6 animate-in fade-in duration-300 pb-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-400 font-semibold">• Tạo bài tập lật ghép 2 hình ảnh</span>
            </div>
            <h1 className="text-xl font-headline font-black text-slate-800 dark:text-white tracking-tight mt-0.5">
              {topicId ? "Chỉnh sửa Game Lật Ảnh-Ảnh" : "Tạo mới Game Lật Ảnh-Ảnh"}
            </h1>
          </div>
        </div>
        ...
        <input
          ...
          className={`w-full px-4 py-3 rounded-2xl border text-sm font-medium transition-all ${
            titleError 
              ...
              : "border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 focus:border-rose-500 focus:bg-white"
          }`}
        />
        {titleError && (
      ...
      <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 overflow-x-auto py-1 scrollbar-none">
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-2xl bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-300 text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap border border-rose-200/60 dark:border-rose-800/40"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {pairs.map((pair, pIdx) => {
              ...
              return (
                <div
                  key={pair.id}
                  className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-all hover:border-rose-200 dark:hover:border-rose-900 flex flex-col justify-between"
                >
                  <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-100 dark:border-slate-800">
                    <span className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <span className="w-5 h-5 rounded-full bg-rose-100 text-rose-600 text-[11px] flex items-center justify-center font-bold">
                        #{pIdx + 1}
                      </span>
                      Cặp Ảnh #{pIdx + 1}
                    </span>
                      ...
                      <div
                        ...
                        className={`relative aspect-square w-full rounded-[4px] border-2 border-dashed overflow-hidden flex flex-col items-center justify-center transition-all ${
                          isDragA 
                            ...
                            : pair.imageAUrl 
                              ? "border-rose-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900" 
                              : "border-slate-300 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900"
                        }`}
                      >
                        {pair.isUploadingA && (
                          <div className="absolute inset-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-[2px] flex flex-col items-center justify-center gap-1.5 z-20">
                        ...
                        {pair.imageAUrl ? (
                          ...
                          <div className="p-2 text-center space-y-1.5">
                            <div className="w-8 h-8 rounded-full bg-rose-50 dark:bg-rose-950/40 text-rose-500 mx-auto flex items-center justify-center">
                            ...
                            <div className="flex items-center justify-center gap-1 pt-0.5">
                              ...
                              <button
                                onClick={() => handleOpenImageSearch(pair, "A")}
                                className="px-2 py-1 rounded-lg bg-rose-500 hover:bg-rose-600 text-white text-[10px] font-bold transition-all flex items-center gap-0.5 shadow-sm"
                              >
            ...
            <div 
              ...
              className={`group border-2 border-dashed border-rose-300 dark:border-rose-800/80 hover:border-rose-500 bg-rose-50/40 dark:bg-rose-950/20 hover:bg-rose-50/80 dark:hover:bg-rose-950/40 rounded-3xl p-6 flex flex-col items-center justify-center text-center space-y-2 transition-all cursor-pointer min-h-[220px] shadow-sm ${
                pairs.length >= MAX_PAIRS_PER_ROUND ? "opacity-50 pointer-events-none" : ""
              }`}
            >
              <div className="w-12 h-12 rounded-full bg-rose-500 text-white flex items-center justify-center shadow-lg shadow-rose-500/30 group-hover:scale-110 transition-transform">
                <Plus className="w-6 h-6 stroke-[3]" />
              </div>
              <span className="font-headline font-black text-sm text-rose-900 dark:text-rose-200">Thêm 1 cặp</span>
              <span className="text-[11px] font-bold text-rose-600/70 dark:text-rose-400/70">
                {pairs.length >= MAX_PAIRS_PER_ROUND 
                  ? `Đã đủ ${MAX_PAIRS_PER_ROUND}/8 cặp cho ${currentRound.title}`
          ...
          <button
            ...
            onClick={handleSave}
            disabled={isSaving}
            className="fixed bottom-6 right-6 z-50 px-6 py-3 sm:px-7 sm:py-3.5 bg-gradient-to-r from-rose-600 via-pink-600 to-rose-700 hover:from-rose-700 hover:to-pink-700 active:scale-95 text-white font-black text-xs sm:text-sm uppercase tracking-wider rounded-full shadow-2xl shadow-rose-900/50 border border-white/30 backdrop-blur-md transition-all duration-300 flex items-center justify-center gap-2.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed group hover:scale-105"
            title="Lưu bài tập"
          >
            {isSaving ? (
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 text-xs font-semibold focus:border-rose-500"
                />
                <button
                  onClick={() => executeImageSearch(searchQuery, imageSearchStyle)}
                  disabled={isSearching}
                  className="px-5 py-2.5 rounded-2xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold transition-all flex items-center gap-1.5"
                >
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-[11px] font-bold transition-all ${
                      imageSearchStyle === "CARTOON" ? "bg-white dark:bg-slate-700 text-rose-600 shadow-sm" : "text-slate-500"
                    }`}
                  >
                    Hoạt hình (Cartoon)
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-[11px] font-bold transition-all ${
                      imageSearchStyle === "REALISTIC" ? "bg-white dark:bg-slate-700 text-rose-600 shadow-sm" : "text-slate-500"
                    }`}
                  >
                    Chân thực (Realistic)
                  </button>
                  ...
                  {searchResults.map((img: any, i: number) => {
                    ...
                    return (
                      <button
                        ...
                        className="group relative aspect-square rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 hover:border-rose-500 hover:scale-105 transition-all shadow-sm bg-slate-900"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-4 text-center">
            ...
            <button
              ...
              className="w-full py-3 rounded-2xl bg-rose-500 hover:bg-rose-600 text-white font-bold text-xs uppercase tracking-wider transition-all"
            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-6 space-y-4 text-center">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => router.push("/teacher?tab=flip")}
      ...
      >
        <GameCardThumbnailPreview
          ...
          pairs={rounds[0]?.pairs.map((p) => ({ word: p.labelB || p.labelA || "Image", imageUrl: p.imageAUrl })) || []}
        />
      </div>
