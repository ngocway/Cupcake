...
function SortableQuestionCard({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`bg-white dark:bg-slate-900 rounded-3xl border border-primary/10 shadow-md p-6 flex flex-col justify-between gap-5 relative group hover:border-amber-400/50 transition-all ${
        isDragging ? "ring-2 ring-amber-500 shadow-2xl scale-[1.02]" : ""
      }`}
    >
      ...
      <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          <button
            ...
            {...attributes}
            {...listeners}
            className="w-7 h-7 rounded-xl bg-slate-100 hover:bg-amber-100 dark:bg-slate-800 dark:hover:bg-amber-950/60 text-slate-400 hover:text-amber-600 transition-all flex items-center justify-center cursor-grab active:cursor-grabbing shrink-0"
            title="Kéo thả để đổi vị trí câu hỏi"
          >
            <GripVertical className="w-4 h-4" />
          </button>
          <span className="w-7 h-7 rounded-xl bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 font-black text-xs flex items-center justify-center">
            #{qIdx + 1}
          </span>
      ...
      <div className="flex gap-3 items-start">
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <textarea
            ...
            className="w-full h-[110px] px-3.5 py-2.5 rounded-[5px] bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white font-bold text-sm focus:outline-none focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all resize-none placeholder:text-slate-400 placeholder:font-normal leading-relaxed"
          />
        </div>
...
export function TreasureHuntCreatorUI() {
  ...
  const handleSaveGame = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await saveTreasureHuntGameAction({
  ...
  return (
    <div className="w-full space-y-6 animate-in fade-in duration-300 pb-28">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className="px-3 py-0.5 rounded-full text-[11px] font-black uppercase tracking-wider bg-gradient-to-r from-amber-500 to-yellow-600 text-white shadow-xs">
                TRUY TÌM KHO BÁU
              </span>
              <span className="text-xs font-semibold text-slate-400">
                • Tạo bài tập trắc nghiệm Bản đồ Kho báu
              </span>
            </div>
            <h1 className="font-headline font-black text-xl sm:text-2xl text-slate-800 dark:text-white tracking-tight">
              {topicId ? "Chỉnh sửa Game Truy tìm Kho báu" : "Tạo mới Game Truy tìm Kho báu"}
            </h1>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-3xl border border-primary/10 shadow-sm flex flex-wrap items-center justify-between gap-3 select-none">
        <div className="flex items-center gap-3 overflow-x-auto select-none no-scrollbar py-0.5">
        ...
        {/* BULK UPLOAD BUTTON (Vibrant Amber - Orange Gradient) */}
        <button
          type="button"
          onClick={() => setIsBulkModalOpen(true)}
          className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:from-amber-600 hover:via-orange-600 hover:to-amber-700 text-white font-bold text-xs uppercase tracking-wider transition-all duration-200 shadow-md shadow-orange-500/25 hover:shadow-lg hover:shadow-orange-500/35 hover:-translate-y-0.5 active:translate-y-0 active:scale-95 border border-amber-400/30 shrink-0 cursor-pointer"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              ...
              {questions.length < MAX_QUESTIONS_PER_ROUND && (
                <button
                  ...
                  className="min-h-[380px] rounded-3xl border-2 border-dashed border-amber-300 dark:border-amber-800/60 hover:border-amber-500 bg-amber-50/30 dark:bg-amber-950/10 hover:bg-amber-50/70 flex flex-col items-center justify-center gap-3 transition-all group cursor-pointer"
                >
                  ...
                  <div className="text-center">
                    <span className="font-headline font-black text-base text-amber-700 dark:text-amber-400 block">
                      + Thêm 1 câu hỏi
                    </span>
                    <span className="text-xs font-semibold text-slate-400">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                  {searchImageModal.results.map((img, i) => (
                    <div
                      ...
                      className="group/item relative aspect-[4/3] rounded-xl overflow-hidden bg-slate-100 cursor-pointer border border-transparent hover:border-amber-500 hover:scale-105 transition-all shadow-xs"
                    >
                      <img
                      ...
                      />
                      <div className="absolute inset-0 bg-amber-500/20 opacity-0 group-hover/item:opacity-100 transition-opacity flex items-center justify-center">
