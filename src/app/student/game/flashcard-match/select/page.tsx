      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10 pt-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
            Click a word, then click the matching image — match all 7 pairs!
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 mb-8 flex-wrap justify-center">
          {LEVEL_ORDER.map((lvl) => {
            ...
            return (
              <Link
                key={lvl}
                href={`/student/game/flashcard-match/select?level=${lvl}`}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-200 ${
                  isActive
                    ? "bg-primary text-white shadow-lg shadow-primary/30 scale-105"
                    : "bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 hover:border-primary/40 hover:text-primary"
                }`}
              >
                <span>{cfg.emoji}</span>
                <span>{cfg.label}</span>
                <span className={`text-xs opacity-70 ${isActive ? "text-white/80" : ""}`}>
                  {cfg.description}
                </span>
              </Link>
            );
