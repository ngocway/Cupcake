...
export async function autoSaveMaterial(payload: { id: string; title: string; type: string; questions: any[]; readingText?: string }) {
  const session = await auth();
  if (!session?.user?.id) throw new Error('Unauthorized');
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (isAutoGenerated) {
      thumbnail = generateMaterialThumbnail(
        { title: payload.title, subject: existing?.subject || 'English' },
        payload.questions
      );
    }

    // Update main info (using upsert to support first-time saves for demo IDs)
