export interface ChoiceEggQuestion {
  id: string;
  typeId: string;
  q: string;
  a: string;
...
export interface ChoiceEggGame {
  id: string;
  code: string;
  title: string;
  ...
  questions: ChoiceEggQuestion[];
  createdAt: string;
  updatedAt: string;
}

const STORAGE_KEY = "dolcake_choice_egg_games";

const DEFAULT_GAMES: ChoiceEggGame[] = [
  {
    id: "EGG-2024",
    code: "EGG-2024",
    title: "Bài tập Đập Trứng Toán học (Mẫu)",
    questionCount: 10,
    endMode: "finish",
...
export function getChoiceEggGames(): ChoiceEggGame[] {
  ...
  try {
    ...
    console.error("Failed to read choice egg games:", e);
    return DEFAULT_GAMES;
  }
}

export function getChoiceEggGameById(idOrCode: string): ChoiceEggGame | null {
  const games = getChoiceEggGames();
...
export function saveChoiceEggGame(
  gameData: Partial<ChoiceEggGame> & { code: string; title: string; questions: ChoiceEggQuestion[] }
): ChoiceEggGame {
  const games = getChoiceEggGames();
  ...
  if (existingIdx >= 0) {
    ...
    savedGame = {
      ...
      code: gameData.code,
      title: gameData.title || "Bài tập Đập Trứng Toán học",
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      console.error("Failed to save choice egg game:", e);
    }
  }
...
export function deleteChoiceEggGame(idOrCode: string): boolean {
  const games = getChoiceEggGames();
  const searchKey = idOrCode.trim().toUpperCase();
  ...
  if (typeof window !== "undefined") {
    try {
      ...
      console.error("Failed to delete choice egg game:", e);
    }
  }
