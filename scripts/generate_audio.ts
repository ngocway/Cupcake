...
// Load .env manually to be 100% robust
function loadEnv() {
...
async function main() {
  ...
  for (const card of cards) {
    ...
    try {
      const audioBuffer = await generateTTS(card.word, card.sentence);
      ...
      console.log(`Updated database for "${card.word}": ${result.count} records`);
    } catch (err: any) {
